...
SERVICE_NAME = "polyhorizon.spark_streaming"
...
def _setup_root_logger() -> None:
    ...
    if LOG_TO_FILE:
        ...
        main_handler = logging.handlers.RotatingFileHandler(
            log_dir / "spark_streaming.log", maxBytes=MAX_FILE_SIZE, backupCount=BACKUP_COUNT, encoding="utf-8"
        )
    ...
    # Quiet noisy third-party loggers common in Spark jobs
    for noisy in ["urllib3", "requests", "boto3", "botocore", "py4j", "pyspark", "kafka", "delta"]:
...
def configure_logging(logging_config) -> None:
    """Configure logger from streaming settings."""
...
def configure_logging_from_settings() -> None:
    """Load streaming settings and configure logging once."""
    ...
    try:
        ...
        logging.getLogger(f"{SERVICE_NAME}.init").warning(
            "Failed to load streaming logging settings; using defaults: %s",
            exc,
        )
